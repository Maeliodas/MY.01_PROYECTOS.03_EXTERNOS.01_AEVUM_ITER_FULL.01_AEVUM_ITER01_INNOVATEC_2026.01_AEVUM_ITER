          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(color: const Color(0xFFEFF3FF), borderRadius: BorderRadius.circular(22)),
                child: Row(children: [const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('ESTADO ACTUAL', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w900, color: Color(0xFF0262FC), letterSpacing: 1)), SizedBox(height: 3)])), Text('Pregunta ${state.currentIndex + 1} de ${state.questions.length}', style: const TextStyle(fontWeight: FontWeight.w700))]),
              ),
              const SizedBox(height: 18),
              SizedBox(width: double.infinity, height: 54, child: ElevatedButton(onPressed: () => Navigator.pop(dialogContext, false), style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: const Color(0xFFFFFFFF), elevation: 2, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(28))), child: const Text('×  Cancelar', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w900)))),
          ...
          decoration: BoxDecoration(
            gradient: LinearGradient(
              ...
              colors: Theme.of(context).brightness == Brightness.dark
                  ...
                  : const [Color(0xFFF2F7FF), Color(0xFFF1F5FF), Color(0xFFE9F3FF)],
            ),
          ),
          child: SafeArea(
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  ClipRRect(borderRadius: BorderRadius.circular(12), child: LinearProgressIndicator(value: state.progress, minHeight: 10, backgroundColor: const Color(0xFFE2E9F1), valueColor: const AlwaysStoppedAnimation(Color(0xFF18A9D3)))),
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Row(mainAxisAlignment: MainAxisAlignment.center, children: [const _ScaleIcon(icon: Icons.block_rounded, label: 'NADA', active: false), const SizedBox(width: 28), Container(width: 78, height: 78, alignment: Alignment.center, decoration: const BoxDecoration(color: Color(0xFFDFEBFF), shape: BoxShape.circle), child: Text(value.round().toString(), style: const TextStyle(fontSize: 38, fontWeight: FontWeight.w900, color: Color(0xFF0262FC)))), const SizedBox(width: 28), const _ScaleIcon(icon: Icons.input_rounded, label: 'MUCHO', active: true)]),
                            const SizedBox(height: 22),
                            SliderTheme(data: SliderTheme.of(context).copyWith(trackHeight: 8, activeTrackColor: AppColors.primary, inactiveTrackColor: const Color(0xFFDCE4F0), thumbColor: AppColors.primary, thumbShape: const RoundSliderThumbShape(enabledThumbRadius: 15), overlayShape: const RoundSliderOverlayShape(overlayRadius: 24)), child: Slider(value: value, min: 0, max: 10, divisions: 10, onChanged: (v) => setState(() => value = v))),
                            ...
                            }, style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: const Color(0xFFFFFFFF), elevation: 3, shadowColor: const Color(0xFF0B3FA3), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30))), child: const Text('Continuar  ›', style: TextStyle(fontSize: 19, fontWeight: FontWeight.w900)))),
                          ],
                        ),
                      ),
                    ),
...
class _ScaleIcon extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) { final dark = Theme.of(context).brightness == Brightness.dark; final secondary = Theme.of(context).colorScheme.onSurface.withValues(alpha: .58); return Column(children: [Container(width: 50, height: 50, decoration: BoxDecoration(color: active ? (dark ? const Color(0xFF1E2C4E) : const Color(0xFFDFEBFF)) : (dark ? const Color(0xFF232E44) : const Color(0xFFEFF2FA)), shape: BoxShape.circle), child: Icon(icon, color: active ? AppColors.primary : secondary)), const SizedBox(height: 7), Text(label, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900, color: active ? AppColors.primary : secondary))]); }
}
